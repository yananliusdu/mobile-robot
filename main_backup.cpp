...
//        box->post_message(VS_MSG_ROUTE_APP,VS_MSG_USER_VALUE,23,sn);
    ...
    packet_switch->case_boundingbox([&](const vs_array<uint8_t>&data){
        ...
        if(arthmetic_product > 1) //return{ 0 0 0 0}, indicates there is no effective data obtained from the scamp
        {
            ...
            if(cornerCount <= 3)
            {
                imageCorner[cornerCount].x = center_x;
                imageCorner[cornerCount].y = center_y;
                cornerCount++;
            }
            else
            {
                ...
                for(int i = 0; i < 4; i++)
                {
                    fprintf(f,"No %d CenterX %f CenterY %f\n",recordEffectiveImage,imageCornerOutput[i].x,imageCornerOutput[i].y);
                }
