    ...
    packet_switch->case_boundingbox([&](const vs_array<uint8_t>&data)
    {
        ...
        if(arthmetic_product > 1) //return{ 0 0 0 0}, indicates there is no effective data obtained from the scamp
        {
            ...
            else
            {
                ...
                int patternCentreX = (imageCorner[0].x + imageCorner[1].x + imageCorner[2].x + imageCorner[3].x)/4;
                int patternCentreY = (imageCorner[0].y + imageCorner[1].y + imageCorner[2].y + imageCorner[3].y)/4;



                for(int i = 0; i < 4; i++)
                {
                    int diffX = imageCorner[i].x - patternCentreX;
                    int diffY = imageCorner[i].y - patternCentreY;
                ...
                center_y = patternCentreY + k_beta*center_y_beta;
                // record these points in txt for later analysis
                //for(int i = 0; i < 4; i++)
                //{
                //    fprintf(f,"%d %f %f %f\n",recordEffectiveImage,imageCornerOutput[i].x,imageCornerOutput[i].y,center_y_beta);
                //}
